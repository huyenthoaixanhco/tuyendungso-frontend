  ...
  return (
    <AuthGuard allowedRoles={['ADMIN']}>
      <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
        <header className="sticky top-0 z-40 flex flex-col gap-3 border-b border-slate-200/60 bg-white/80 px-4 py-3 shadow-sm backdrop-blur-md md:flex-row md:items-center md:justify-between md:px-6 md:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 md:gap-3">
              <div className="rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 p-1.5 md:p-2 shadow-lg shadow-emerald-500/20">
                <Shield className="text-white" size={20} strokeWidth={2.5} />
              </div>
              <div>
                <h1 className="leading-none text-lg font-extrabold tracking-tight text-slate-900 md:text-xl">Tuyendungso.vn</h1>
                <p className="mt-1 text-[10px] font-semibold uppercase tracking-wider text-emerald-600 md:text-xs">Cổng Quản Trị</p>
              </div>
            </div>
          ...
          <nav className="scrollbar-hide flex w-full items-center gap-2 overflow-x-auto pb-1 md:w-auto md:gap-1 md:pb-0">
            {(Object.keys(tabConfig) as AdminTab[]).map((key) => {
              ...
              return (
                ...
                >
                  <TabIcon size={14} strokeWidth={isActive ? 2.5 : 2} className="md:w-4 md:h-4" />
                  {tabConfig[key].title}
                </button>
              );
            })}
        ...
        <main className="mx-auto max-w-7xl animate-in space-y-6 fade-in slide-in-from-bottom-4 p-4 duration-500 md:space-y-8 md:p-6">
          <section className="flex flex-col items-start justify-between gap-2 md:flex-row md:items-end md:gap-4">
            <div>
              <h2 className="mb-1 flex items-center gap-2 text-2xl font-black tracking-tight text-slate-900 md:mb-2 md:gap-3 md:text-3xl">{pageTitle}</h2>
          ...
          {(activeTab === 'overview' || activeTab === 'reports') && (
            <div className="grid gap-6 animate-in fade-in duration-500 xl:grid-cols-3">
                  ...
                  <div className="space-y-4 p-4 md:p-6">
                    {dashboard.latestPendingJobs.length === 0 ? (
                      <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 px-4 py-8 text-center md:py-10">
                        <CheckCircle2 size={32} className="mx-auto mb-2 text-emerald-400 md:w-10 md:h-10 md:mb-3" />
                        ...
                        >
                          <div>
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-slate-600 md:mt-1.5 md:gap-2 md:text-sm">
                              <Building2 size={12} className="text-slate-400 md:w-[14px] md:h-[14px]" />
              ...
              <div className="sticky top-28 h-fit rounded-3xl bg-gradient-to-b from-slate-900 to-slate-800 p-6 text-white shadow-xl md:p-8">
                <div className="mb-5 flex items-center gap-2 md:mb-6 md:gap-3">
                  <div className="rounded-xl bg-emerald-500/20 p-2">
                    <Bot size={24} className="text-emerald-400 md:w-7 md:h-7" />
                  </div>
                ...
                <div className="space-y-3">
                  <h4 className="mb-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 md:mb-3 md:text-xs">Đề xuất hành động</h4>
                  {aiInsight.recommendedActions.length > 0 ? (
                    aiInsight.recommendedActions.map((item, idx) => (
                      <div key={idx} className="flex items-start gap-2 rounded-xl border border-white/10 bg-white/5 p-3 transition-colors hover:bg-white/10 md:gap-3 md:p-4">
                        <ChevronRight size={16} className="mt-0.5 shrink-0 text-emerald-400 md:w-[18px] md:h-[18px]" />
          ...
          {activeTab === 'users' && (
            <section className="overflow-hidden rounded-3xl border border-slate-200/60 bg-white shadow-sm animate-in fade-in duration-500">
              <div className="flex flex-col gap-3 border-b border-slate-100 bg-slate-50/50 p-4 sm:flex-row sm:p-6">
                <div className="relative flex-1">
                  <Search size={16} className="absolute left-3.5 top-3 text-slate-400 md:top-3.5 md:w-[18px] md:h-[18px]" />
                  <input
                    value={userKeyword}
                    onChange={(e) => setUserKeyword(e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && loadUsers()}
                    placeholder="Tìm kiếm email, tên..."
                    className="w-full rounded-2xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-xs font-medium outline-none transition-all shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 md:py-3 md:text-sm"
                  />
                </div>
                <div className="flex gap-2 md:gap-3">
                  <select
                    ...
                    className="w-1/2 sm:w-auto cursor-pointer rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-medium outline-none transition-all shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 md:px-5 md:py-3 md:text-sm"
                  >
                  ...
                  <button
                    onClick={loadUsers}
                    className="w-1/2 sm:w-auto rounded-2xl bg-slate-900 px-4 py-2.5 text-xs font-bold text-white shadow-sm transition-all hover:bg-slate-800 hover:shadow-md active:scale-95 md:px-6 md:py-3 md:text-sm"
                  >
                    Lọc
                  </button>
                </div>
                        ...
                        <td className="px-4 py-3 md:px-6 md:py-4">
                          {user.verified ? (
                            <span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600 md:text-sm">
                              <CheckCircle2 size={14} className="md:w-4 md:h-4" /> Xác thực
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-xs font-medium text-slate-400 md:text-sm">
                              <XCircle size={14} className="md:w-4 md:h-4" /> Chưa XT
                            </span>
                          )}
                            ...
                            <div className="mt-1 md:mt-2">
                              {user.employerApproved ? (
                                <span className="inline-flex items-center gap-1 rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-semibold text-blue-600 ring-1 ring-inset ring-blue-600/20 md:px-2 md:py-1 md:text-xs">
                                  <Building2 size={10} className="md:w-3 md:h-3" /> Đã duyệt CTy
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 rounded bg-amber-50 px-1.5 py-0.5 text-[10px] font-semibold text-amber-600 ring-1 ring-inset ring-amber-600/20 md:px-2 md:py-1 md:text-xs">
                                  <Building2 size={10} className="md:w-3 md:h-3" /> Chờ duyệt CTy
                                </span>
                              )}
                            </div>
                          )}
                        ...
                        <td className="px-4 py-3 text-right md:px-6 md:py-4">
                          <div className="flex flex-col sm:flex-row justify-end gap-2">
                            <button
                              ...
                              className={`inline-flex items-center justify-center gap-1 rounded-lg px-2 py-1 text-xs font-bold transition-all md:px-3 md:py-1.5 md:text-sm md:gap-1.5 ${
                                user.active
                            ...
                            >
                              {user.active ? <Lock size={12} className="md:w-[14px] md:h-[14px]"/> : <Unlock size={12} className="md:w-[14px] md:h-[14px]"/>}
                    ...
                    {users.length === 0 && !loading && (
                      <tr>
                        <td colSpan={6} className="py-10 text-center md:py-16">
                          <Users size={40} className="mx-auto mb-3 text-slate-200 md:w-12 md:h-12 md:mb-4" />
              ...
              <div className="space-y-4 bg-slate-50/30 p-4 md:p-6">
                  ...
                  <div key={job.id} className="rounded-2xl border border-slate-200 bg-white p-4 transition-all duration-300 hover:border-emerald-200 hover:shadow-lg md:p-6">
                    <div className="flex flex-col gap-4 xl:flex-row xl:gap-6">
                      <div className="flex-1 space-y-3 md:space-y-4">
                          ...
                          <div className="mb-2 flex flex-wrap items-center gap-2 md:gap-3">
                            ...
                            {job.aiReviewed && (
                              <span className="flex items-center gap-1 rounded-full bg-gradient-to-r from-purple-50 to-indigo-50 px-2 py-0.5 text-[10px] font-bold text-purple-700 ring-1 ring-inset ring-purple-600/20 md:px-3 md:py-1 md:text-xs">
                                <Sparkles size={10} className="md:w-3 md:h-3"/> AI đánh giá
                              </span>
                            )}
                          </div>

                          <p className="flex flex-wrap items-center gap-1.5 text-xs font-medium text-slate-600 md:gap-2 md:text-sm">
                            <Building2 size={14} className="text-slate-400 md:w-4 md:h-4" />
                            {job.companyName}
                            <span className="text-slate-300">•</span>
                            {job.location || '—'}
                          </p>
                        ...
                        <div className="grid gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3 grid-cols-2 md:grid-cols-4 md:p-4 md:gap-4">
                          <div>
                        ...
                        {job.aiSummary && (
                          <div className="rounded-xl border border-purple-100 bg-purple-50/50 p-3 text-xs leading-relaxed text-slate-700 md:p-4 md:text-sm">
                            <span className="mb-1 flex items-center gap-1 font-bold text-purple-800">
                              <Bot size={14} className="md:w-4 md:h-4" /> Tóm tắt từ AI:
                            </span>
                            {job.aiSummary}
                          </div>
                        )}
                      ...
                      <div className="flex shrink-0 flex-row flex-wrap justify-end gap-2 border-t border-slate-100 pt-3 md:gap-3 md:pt-4 xl:w-56 xl:flex-col xl:justify-start xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
                        ...
                        >
                          <Sparkles size={14} className="md:w-4 md:h-4"/> Đánh giá AI
                        </button>
                ...
                {jobs.length === 0 && !loading && (
                  <div className="rounded-3xl border-2 border-dashed border-slate-200 bg-white py-12 text-center md:py-20">
                    <Briefcase size={40} className="mx-auto mb-3 text-slate-300 md:w-12 md:h-12 md:mb-4" />
          ...
          {activeTab === 'settings' && (
            <section className="grid gap-4 animate-in fade-in duration-500 sm:grid-cols-2 md:gap-6 xl:grid-cols-3">
              {settings.map((item) => (
                <div
                  key={item.key}
                ...
                >
                  <div className="mb-4 md:mb-5">
                    <div className="mb-2 flex items-center gap-1.5 md:gap-2">
                      <Settings size={16} className="text-emerald-500 md:w-[18px] md:h-[18px]" />
          ...
          {activeTab === 'ai' && (
            <section className="space-y-6 animate-in fade-in duration-500 md:space-y-8">
              <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 p-6 text-white shadow-2xl sm:p-8 md:p-12">
                <div className="pointer-events-none absolute right-0 top-0 -mr-8 -mt-8 rotate-12 text-white/5 sm:-mr-16 sm:-mt-16">
                  <Bot size={200} strokeWidth={1} className="md:w-[300px] md:h-[300px]" />
                </div>
                ...
                <div className="relative z-10 max-w-2xl">
                  <div className="mb-4 inline-flex items-center gap-1.5 rounded-full border border-indigo-400/30 bg-indigo-500/20 px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-indigo-200 md:mb-6 md:gap-2 md:px-3 md:text-xs">
                    <Sparkles size={12} className="md:w-3.5 md:h-3.5" /> Hệ thống tự động hóa
                  </div>
                  ...
                  >
                    <Sparkles size={18} className="text-indigo-600 md:w-5 md:h-5" />
              ...
              <div className="rounded-3xl border border-slate-200/60 bg-white p-5 shadow-sm md:p-8">
                <div className="mb-5 flex items-center gap-2 md:mb-6 md:gap-3">
                  <div className="rounded-xl bg-purple-100 p-2 md:p-2.5">
                    <Bot size={20} className="text-purple-600 md:w-6 md:h-6" />
                  </div>
    ...
    >
      ...
      {highlight && (
        <div className="absolute -bottom-2 -right-2 rotate-12 opacity-10 md:-bottom-4 md:-right-4">
          <Activity size={60} strokeWidth={3} className="md:w-[80px] md:h-[80px]" />
        </div>
      )}
    </div>
  );
